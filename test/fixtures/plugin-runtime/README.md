...
`scripts/plugin-runtime-probe.mjs` copies the tracked Marketplace template from `.agents/plugins/marketplace.json` and `plugins/kyoso/` into a temporary root. It replaces only the temporary `.codex-plugin/mcp.json` command with `probe-server.mjs`, then runs Codex with distinct temporary `HOME`, `CODEX_HOME`, and workspace directories.
